...
function viewInvoice(orderId) {
  ...
  setInv('invoice-restaurant-name',    settings.restaurantName || 'Restaurant');
  ...
  setInv('invoice-table',    order.tableName);
...
<head>
  <meta charset="UTF-8" />
  <title>Invoice — ${settings.restaurantName || 'Restaurant'}</title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .invoice-header { text-align: center; padding-bottom: 14px; margin-bottom: 14px; border-bottom: 1px solid #e2e8f0; }
    ...
    .invoice-meta { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 6px; padding: 12px; margin-bottom: 14px; }
    .invoice-meta-label { font-size: 10px; font-weight: 600; text-transform: uppercase; color: #94a3b8; margin-bottom: 2px; }
    .invoice-meta-value { font-size: 13px; font-weight: 500; }
    ...
    .invoice-footer { text-align: center; font-size: 12px; color: #94a3b8; margin-top: 18px; padding-top: 12px; border-top: 1px solid #f1f5f9; }
